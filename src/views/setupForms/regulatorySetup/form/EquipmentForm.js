...
const EquipmentTypeForm = ({ openModal, title, setOpenModal }) => {
 ...
 const { regulatoryOption} = useSelector(state => state.regulatoryOptionState)

 const PostData = async (data) => {
  dispatch(showLoader());
  
  const response = await customApi.post("/RegulatoryOptions", data);
  console.log({response})
};
const PutData = async (data) => {
  dispatch(showLoader());
  
  const response = await customApi.put("/RegulatoryOptions",data);
  console.log({response})
};
const DeleteData = async (data) => {
  dispatch(showLoader());
  const response = await customApi.delete("/RegulatoryOptions", {
    data: { regulatoryOptionId: data.id },
  });
  console.log({response})
};
...
const onPostSuccess = () => {
  ...
  queryClient.invalidateQueries("regulatoryOptions");
  setOpenModal(false);
...
const handleSubmit = (values) => {
  ...
  regulatoryOption?.id ? putData({...values, regulatoryOptionId:regulatoryOption.id}) : postData(values);
};
  ...
  const onConfirm = () => {
    ...
    deleteData(regulatoryOption)

    // dispatch(deleteRegulatoryOptions(regulatoryOption))
    setOpenModal(false);
  };
          ...
          <Row>
            <Col sm="12">
              <div className="doc-title mt-10">
                <Formik
                  initialValues={regulatoryOption}
